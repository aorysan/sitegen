...
1. Jalankan skrip screenshot crawler (baca port dev server secara dinamis dan ambil daftar rute dari `PAGES-LIST.md` / `PRD.md`):
...
2. **STRICT AUTO-FAIL**: Skrip Puppeteer di atas WAJIB menangkap dan memunculkan error hard-fail jika mendeteksi:
   ...
   - Tag `<img>` atau `<Image>` yang tidak memiliki atribut `alt` atau `title`, atau tidak responsif (`max-width: 100%`).
   - 404 network error pada pemuatan gambar (broken image links) atau penggunaan gambar placeholder (`picsum.photos`).
...
1. Lakukan pengecekan terhadap kepatuhan aturan `generator`. Agen **WAJIB memverifikasi secara manual**:
   ...
   - **Animasi Scroll Anime.js**: Pastikan elemen/section menggunakan `animejs` (komponen `AnimatedSection.tsx` berbasis `IntersectionObserver` dan `anime.remove()`), animasi HANYA terpicu saat scroll dari atas ke bawah dan menggunakan Asymmetric Reset (tidak terulang saat scroll dari bawah ke atas). DILARANG MENGGUNAKAN `framer-motion` ATAU ANIMASI SCROLL CSS MURNI.
