...
5. **Fase Eksekusi Halaman (Hybrid SDD Iteration):**
   ...
   a. **AI-to-AI SDD Loop:**
      ...
      - **Kepatuhan Arsitektur & Aturan Eksekusi Wajib Generator:**
        ...
        3. **Mandatory SEO Image Attributes**: Saat menggunakan komponen gambar (`<img />` atau `next/image`), Generator WAJIB MENYERTAKAN pasangan atribut `alt="..."` DAN `title="..."` dengan untaian kata kunci (keyword) SEO yang akurat.
...
7. **Fase SEO Validation & Debug Lokal Final**:
   a. **SEO Validation**: Panggil sub-skill `seo-validator` dan perintahkan untuk membaca `landings/<brand>/planning/PLAN-USER-NEEDS.md`, `PLAN-COMPETITOR.md`, serta `PRD.md`. Ekstrak keyword yang relevan dan lakukan validasi SEO secara komprehensif pada setiap halaman web yang dihasilkan. Dalam audit ini, validator WAJIB menginspeksi secara eksplisit seluruh tag `<img />` dan komponen gambar `next/image`: PASTIKAN setiap tag terbekali dengan kedua atribut `alt="..."` dan `title="..."` dengan untai teks yang kaya makna SEO spesifik brand (tanpa teks hampa/kosong).
   b. **Debug Lokal Final & Sanity QA**: Sebelum menyerahkan hasil akhir kepada user, wajib melakukan debug lokal dengan menjalankan build produksi secara mandiri via tools di latar belakang:
      - Jalankan terminal: `cd landings/<brand> && npm run build` (atau perintah build setara sesuai `AGENTS.md`). Verifikasi build berhasil tanpa error statik maupun TypeScript.
      - Panggil kembali `qa-reviewer` untuk melakukan uji kewarasan (*sanity test*): **Verifikasi Eksplisit Terakhir**: cek keberfungsian interaksi mobile pada komponen pemutar video/media (*touch screen accessible*, memiliki kendali navigasi tekan jelas tanpa terjeda lock interaktif) serta pastikan Top Navbar konstan merekat melayang dengan gaya kelas CSS `sticky top-0` yang tidak terhalangi overflow container lain di semua breakpoint resolusi uji.

8. **[CRITICAL STOP - TUNGGU REVIEW USER]**: Tampilkan laporan hasil validasi SEO dan bukti keberhasilan build lokal akhir kepada user. **HARD STOP**: BERHENTI MENGEKSEKUSI TOOL APA PUN. Tunggu user mereview hasil akhir di dev server / lokal dan memberikan persetujuan eksplisit sebelum melanjutkan ke tahap deploy.
