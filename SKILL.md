   ...
   **Strategi Eksekusi Multi-Subagent:**
   ...
   a. **AI-to-AI SDD Loop & Fallback Protocol:**
      ...
      - **Kepatuhan Arsitektur & Aturan Eksekusi Wajib Generator (`AGENTS.md` Compliance):**
       ...
       - Setelah pembangunan halaman selesai, panggil subagent `qa-reviewer` mode=code-review untuk mengecek hasil kode `.tsx` terhadap PRD, *Strict Slug*, keberadaan logo & gambar nyata yang relevan, serta verifikasi bahwa animasi scroll HANYA terpicu saat scroll ke bawah (bukan bidirectional, bukan `once: true`).
       - Jika QA menemukan bug, ORKESTRATOR (Anda) WAJIB menghitung iterasi dan mendelegasikan perbaikan ulang ke generator, maksimal 5 iterasi.
